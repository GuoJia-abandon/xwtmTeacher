package com.guo.department.service;

import com.guo.base.pojo.vo.PageVo;
import com.guo.pojo.entity.department.Course;

import java.util.List;

/**
 * 类描述：课程信息业务逻辑层接口
 *
 * @ClassName CourseService
 * @Description 课程信息业务逻辑层接口
 * @Author 郭佳
 * @Date 2021/3/20 21:54
 * @Version 1.0
 */
public interface CourseService {
	/** 
	 * @Description: 根据id获取对象信息
	 * @Param: [java.lang.String]
	 * @return: com.guo.pojo.entity.Course.Course
	 * @Author: 郭佳
	 * @Date: 2021/3/20
	 */
	Course getCourseById(String id) throws Exception;

	/**
	 * @Description: 通过查询条件查询对象分页信息
	 * @Param: [com.guo.pojo.entity.Course.Course, com.guo.base.pojo.vo.PageVo]
	 * @return: com.guo.base.pojo.vo.PageVo<java.lang.Object>
	 * @Author: 郭佳
	 * @Date: 2021/3/21
	 */
	PageVo<Course> getPageVoByQuery(Course query,PageVo<Course> pageVo) throws Exception;

	/**
	 * @Description: 根据查询条件查询对象信息
	 * @Param: [com.guo.pojo.entity.Course.Course]
	 * @return: java.util.List<com.guo.pojo.entity.Course.Course>
	 * @Author: 郭佳
	 * @Date: 2021/3/21
	 */
	List<Course> getListByQuery(Course query) throws Exception;

	/**
	 * @Description: 保存对象信息
	 * @Param: [com.guo.pojo.entity.Course.Course]
	 * @return: boolean
	 * @Author: 郭佳
	 * @Date: 2021/3/21
	 */
	boolean save(Course entity) throws Exception;

	/**
	 * @Description: 更新对象信息
	 * @Param: [com.guo.pojo.entity.Course.Course]
	 * @return: boolean
	 * @Author: 郭佳
	 * @Date: 2021/3/21
	 */
	boolean update(Course entity) throws Exception;


}