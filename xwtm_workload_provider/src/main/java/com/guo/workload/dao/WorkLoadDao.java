package com.guo.workload.dao;
import com.guo.pojo.entity.workload.WorkLoad;
import org.springframework.stereotype.Repository;

import java.util.List;

/**
 * 类描述：工作量信息持久层接口
 *
 * @ClassName WorkLoadDao
 * @Description 工作量信息持久层接口
 * @Author 郭佳
 * @Date 2021/3/20 20:57
 * @Version 1.0
 */
@Repository
public interface WorkLoadDao {

	/**
	 * @Description: 根据条件查询列表信息
	 * @Param: [com.guo.pojo.entity.WorkLoad.WorkLoad]
	 * @return: java.util.List<com.guo.pojo.entity.WorkLoad.WorkLoad>
	 * @Author: 郭佳
	 * @Date: 2021/3/20
	 */
	 List<WorkLoad> findListByQuery(WorkLoad query);

	 /**
	  * @Description: 保存信息
	  * @Param: [com.guo.pojo.entity.WorkLoad.WorkLoad]
	  * @return: int
	  * @Author: 郭佳
	  * @Date: 2021/3/20
	  */
	 int save(WorkLoad entity) throws Exception;

	 /**
	  * @Description: 更新信息
	  * @Param: [com.guo.pojo.entity.WorkLoad.WorkLoad]
	  * @return: int
	  * @Author: 郭佳
	  * @Date: 2021/3/20
	  */
	 int update(WorkLoad entity) throws Exception;

}